defmodule MyList do
  ...
  #fastest
  def mapt(l,f), do: mapt(l,[],f)
  def mapt([],r,f), do: Enum.reverse r
  def mapt([h|t],r,f), do: mapt(t,[f.(h)|r],f)

  #1158.32x slower
  def maptpp(l,f), do: maptpp(l,[],f)
  def maptpp([],r,f), do: r
  def maptpp([h|t],r,f), do: maptpp(t,r ++ [f.(h)],f)
  ...
  #For filtering this body recursive version is about 350x faster
  #than the tail recursive ones...
  def filter([],_f), do: []
  def filter([h|t],f) do
    if !f.(h) do
      filter(t,f)
  ...
  #appending with ++ concatenation operator HORRIBLY (2,500x) slow
  def filter2(l,f), do: filter2p(l,[],f)
  def filter2p([],r,f), do: r
  ...
  #tail recursive version, 350x slower than body recursive.. why?
  ...
  #tail recursive version no reverse (result is reversed.)
  def filter4(l,f), do: filter2p(l,[],f)
  def fllter4p([],r,f), do: r
  def filter4p([h|t],r,f) do
    if !f.(h) do
      filter4p(t,r,f)
    else
      filter4p(t,[h|r],f)
...
defmodule User do
  ...
  i filter2(list,&Integer.is_even/1)
